...
export const ToggleNav = ({ title, isOpen, setIsOpen, children }: PropsWithChildren<ToggleNavProps>) => {
    const toggleOpen = () => setIsOpen((prev) => !prev)

    return (
        <nav onClick={toggleOpen} className="relative w-full p-6 bg-slate-800 rounded-2xl text-start">
            <div className="w-full">
                <h3 className="w-[87.5%] text-lg text-slate-50 ">{title}</h3>
                <div
